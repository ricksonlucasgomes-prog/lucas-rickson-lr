...
- **DONE (2026-07-26, loop iter 1): remaining audit fixes in `styles.css`** (flagged to Codex earlier, applied as agreed-fix one-liners): outlined-headline spans now sit inside `@supports (-webkit-text-stroke: …)` so engines without the non-standard property render normal filled text instead of invisible copy (also migrated their last `237 230 245` literals to steel `232 237 242`); filtered projects grid normalizes card media to `aspect-ratio: 4/3` at both 40rem and 72rem (per-position mosaic ratios no longer pair 3/4 beside 16/8); deleted the unreferenced `signal-pulse` keyframes. Verified: build clean, filtered "Vídeo & VFX" renders two uniform 4/3 cards, outlined spans still render in Chromium. Still open for Codex: the double-rAF guard in `main.js` (`handleMotionPreference` can start two loops).
